...
(window.PROBLEMS_REGISTRY = window.PROBLEMS_REGISTRY || []).push({
  ...
  difficulty: 5,
  language: 'proof',
  description: '【発展定理（規則汎化の必要十分条件）】\n$d=r+D$、$u\\in U\\cap\\mathbb S^{d-1}$ とし、\n$$W_\\lambda=P_U+\\lambda P_{U^\\perp},\\qquad 0\\le\\lambda\\le1$$\nを考える。文脈平均から得られる法線が\n$$W_\\lambda\\widehat v=(q_\\varepsilon+\\overline\\xi)u+H_\\lambda$$\nと分解され、$H_\\lambda\\perp u$、\n$$\\lVert H_\\lambda\\rVert_2^2\\overset d=\\frac1M\\left(\\chi^2_{r-1}+\\lambda^2\\chi^2_D\\right),\\qquad \\overline\\xi\\xrightarrow{\\mathbb P}0$$\nを満たすとする。このとき規則誤差が $0$ へ確率収束するための必要十分条件は\n$$\\eta_M=\\frac{r-1+\\lambda^2D}{M}\\longrightarrow0$$\nである。以下の証明ステップを正しい順序に並び替えよ。\n\n【記号】$d$ は入力次元、$M$ は文脈中の標本数である。$U\\subset\\mathbb R^d$ は次元 $r$ の共有規則部分空間、$D=d-r$ はその直交補空間の次元、$u$ は $U$ 内の単位教師方向である。$P_U,P_{U^\\perp}$ は各部分空間への直交射影で、$\\lambda$ は事実記憶に使う $U^\\perp$ 方向の固有値である。$\\widehat v=M^{-1}\\sum_{i=1}^M y_ix_i$ は文脈平均、$q_\\varepsilon=(1-\\varepsilon)\\sqrt{2/\\pi}$ は平均規則信号、$\\overline\\xi$ は教師方向の標本ゆらぎ、$H_\\lambda$ は教師方向に直交する雑音である。$\\chi_k^2$ は自由度 $k$ のカイ二乗分布、$\\overset d=$ は分布同値、$\\xrightarrow{\\mathbb P}$ は確率収束を表す。規則誤差 $\\mathcal E_{\\rm rule}(\\lambda)$ は、独立な $x_{\\rm new}\\sim\\mathcal N(0,I_d)$ に対する分類器 $\\operatorname{sign}(\\widehat v^\\top W_\\lambda x_{\\rm new})$ と教師 $\\operatorname{sign}(u^\\top x_{\\rm new})$ の不一致確率である。',
  pinnedCode: ['【証明】'],
  blocks: [
